        ...
        gameStatus.addRowInfo({
          ...
          rowInfo: {
            getScoreFrom: ({ handInputMap }) => {
              ...
              if (handInputA === undefined) throw new Error(`No such hand: NUMBERS_${curr[0]}`);
              if (handInputB === undefined) throw new Error(`No such hand: NUMBERS_${curr[1]}`);

              if (handInputA === null || handInputB === null)
                return 0;
